  ...
  return (
    <main style={page}>
      <section style={shell}>
        <div style={topBar}>
          <PageHeader
            eyebrow={<Link href="/students" style={backLink}>학생 현황판</Link>}
            title="학생 상세 정보"
            description={`${student.name} 학생의 기본 정보, 메모, 성적, 상담 이력을 한 화면에서 관리합니다.`}
            meta={
              <div className="asc-status-row">
                <StatusBadge status={student.status} label={statusText[student.status] ?? student.status} />
                <Badge tone="gray">{primaryClass ? `${primaryClass.teacher?.name ? `${primaryClass.teacher.name} / ` : ""}${primaryClass.name}` : "반 미지정"}</Badge>
              </div>
            }
            actions={
        ...
        <div style={detailGrid}>
          <aside style={leftColumn}>
            <section style={profileCard}>
              <form action={updateStudent} style={profileForm}>
                ...
                <div style={profileHead}>
                  ...
                  <div>
                    ...
                    <div style={tagRow}>
                      <StatusBadge status={student.status} label={statusText[student.status] ?? student.status} />
                      {student.grade && <Badge tone="gray">{student.grade}</Badge>}
                      {student.subject && <Badge tone="gray">{student.subject}</Badge>}
                      {student.currentLevel && <Badge tone="gray">{student.currentLevel}</Badge>}
                    </div>
                  </div>
              ...
              <form action={deleteStudent} style={deleteForm} className="asc-danger-zone">
                <input type="hidden" name="studentId" value={student.id} />
...
const dangerButton: CSSProperties = { height: 32, border: "1px solid var(--asc-danger)", borderRadius: "var(--asc-radius-md)", color: "var(--asc-danger)", background: "var(--asc-surface)", padding: "0 12px", fontWeight: 900 };
