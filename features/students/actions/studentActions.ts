...
import { ClassGroupStatus, MemoType, Prisma, StudentStatus, StudentTestScoreSource } from "@/lib/generated/prisma";
...
export async function updateStudentLessonCells(formData: FormData) {
  const user = await requireUser();
  const rawCells = text(formData, "cells");
  const selectedClassTestId = cleanId(text(formData, "selectedClassTestId"));
  const selectedClassGroupId = cleanId(text(formData, "selectedClassGroupId"));
  ...
  if (!Array.isArray(parsed)) throw new Error("Invalid lesson cells.");
  ...
  const cells = parsed
    .map((item) => {
      ...
      const examId = cleanId(String(raw.examId ?? ""));
      const classTestId = cleanId(String(raw.classTestId ?? ""));
      const classLessonId = cleanId(String(raw.classLessonId ?? ""));
      const lessonPositionNumber = Number(raw.lessonPosition);
      const lessonPosition = Number.isInteger(lessonPositionNumber) && lessonPositionNumber > 0 ? lessonPositionNumber : null;
      if (!studentId) return null;
      if (!["attendance", "assignment", "test"].includes(field)) return null;
      if (field !== "test" && !/^\d{4}-\d{2}-\d{2}$/.test(date)) return null;
      if (field === "test" && !examId && !classTestId) return null;
      return { studentId, date, field, value, examId, classTestId, classLessonId, lessonPosition };
    })
    .filter(
      (cell): cell is {
        studentId: string;
  ...
  if (cells.length === 0) throw new Error("No lesson cells to save.");

  const studentIds = Array.from(new Set(cells.map((cell) => cell.studentId)));
  const examIds = Array.from(new Set(cells.map((cell) => cell.examId).filter((examId): examId is string => Boolean(examId))));
  const classTestIds = Array.from(new Set(cells.map((cell) => cell.classTestId).filter((classTestId): classTestId is string => Boolean(classTestId))));
  const classLessonIds = Array.from(new Set(cells.map((cell) => cell.classLessonId).filter((classLessonId): classLessonId is string => Boolean(classLessonId))));

  const [students, selectedExams, selectedClassTests, classLessons] = await Promise.all([
    prisma.student.findMany({ where: { id: { in: studentIds }, academyId: user.academyId }, select: { id: true } }),
    examIds.length > 0
      ? prisma.exam.findMany({
          where: { id: { in: examIds }, academyId: user.academyId },
          select: { id: true, classGroupId: true, classTestId: true, classLessonId: true, lessonPosition: true, totalScore: true, questionCount: true },
        })
      : Promise.resolve([]),
    classTestIds.length > 0
      ? prisma.classTest.findMany({
          where: { id: { in: classTestIds }, academyId: user.academyId, active: true },
          select: {
            id: true,
            classGroupId: true,
            classLessonId: true,
    ...
    classLessonIds.length > 0
      ? prisma.classLesson.findMany({
          where: { id: { in: classLessonIds }, academyId: user.academyId },
          select: { id: true, classGroupId: true, position: true, lessonDate: true },
        })
      : Promise.resolve([]),
  ]);
  const examById = new Map(selectedExams.map((exam) => [exam.id, exam]));
  const classTestById = new Map(selectedClassTests.map((classTest) => [classTest.id, classTest]));
  const classLessonById = new Map(classLessons.map((lesson) => [lesson.id, lesson]));
  const allowedStudentIds = new Set(students.map((student) => student.id));
  const allowedCells = cells.filter((cell) => allowedStudentIds.has(cell.studentId));

  if (allowedCells.length === 0) throw new Error("No students to save.");
  ...
  await prisma.$transaction(async (tx) => {
    for (const cell of allowedCells) {
      ...
      if (cell.field === "assignment") {
        if (!cell.value) {
          await tx.assignmentRecord.deleteMany({ where: { studentId: cell.studentId, date: cell.date, title: "??" } });
        } else {
          await tx.assignmentRecord.upsert({
            where: { studentId_date_title: { studentId: cell.studentId, date: cell.date, title: "??" } },
            update: { status: cell.value },
            create: { academyId: user.academyId, studentId: cell.studentId, date: cell.date, title: "??", status: cell.value },
          });
        }
      }

      if (cell.field === "test") {
        let selectedExam = cell.examId ? examById.get(cell.examId) ?? null : null;
        const classTest = cell.classTestId ? classTestById.get(cell.classTestId) ?? null : null;

        if (!selectedExam && classTest) {
          if (selectedClassGroupId && classTest.classGroupId !== selectedClassGroupId) throw new Error("Selected test does not belong to the selected class.");
          if (selectedClassTestId && classTest.id !== selectedClassTestId) throw new Error("Selected score cell does not belong to the selected test.");
          const linkedLesson = cell.classLessonId ? classLessonById.get(cell.classLessonId) ?? null : null;
          const lessonPosition = linkedLesson?.position ?? cell.lessonPosition ?? classTest.lessonPosition ?? null;
          const lessonIdForDb = linkedLesson?.classGroupId === classTest.classGroupId ? linkedLesson.id : classTest.classLessonId;

          const examLessonFilters = [
            ...(lessonIdForDb ? [{ classLessonId: lessonIdForDb }] : []),
            ...(lessonPosition ? [{ lessonPosition }] : []),
          ];
          selectedExam = examLessonFilters.length > 0
            ? await tx.exam.findFirst({
                where: {
                  academyId: user.academyId,
                  classGroupId: classTest.classGroupId,
                  classTestId: classTest.id,
                  OR: examLessonFilters,
                },
                select: { id: true, classGroupId: true, classTestId: true, classLessonId: true, lessonPosition: true, totalScore: true, questionCount: true },
              })
            : null;

          if (!selectedExam && cell.value) {
            const examTitle = lessonPosition ? String(lessonPosition) + "\uCC28\uC2DC " + classTest.name : classTest.name;
            selectedExam = await tx.exam.create({
              data: {
                academyId: user.academyId,
                classGroupId: classTest.classGroupId,
                classTestId: classTest.id,
                classLessonId: lessonIdForDb ?? null,
                lessonPosition,
                title: examTitle,
                subject: classTest.subject,
                examDate: linkedLesson?.lessonDate ?? null,
                templateType: classTest.templateType,
                questionCount: classTest.questionCount ?? 20,
        ...
        if (selectedExam?.classGroupId && selectedExam.classTestId) {
          if (selectedClassGroupId && selectedExam.classGroupId !== selectedClassGroupId) throw new Error("Selected test does not belong to the selected class.");
          if (selectedClassTestId && selectedExam.classTestId !== selectedClassTestId) throw new Error("Selected score cell does not belong to the selected test.");
          if (!cell.value) {
            await tx.studentTestScore.deleteMany({ where: { studentId: cell.studentId, examId: selectedExam.id } });
          } else {
            const numericScore = Number(cell.value);
            const score = Number.isFinite(numericScore) ? Math.round(numericScore) : null;
            const totalScore = selectedExam.totalScore ?? selectedExam.questionCount ?? 100;
            await tx.studentTestScore.upsert({
              where: { studentId_examId: { studentId: cell.studentId, examId: selectedExam.id } },
              update: { score, totalScore, source: StudentTestScoreSource.MANUAL, memo: cell.value },
              create: {
                academyId: user.academyId,
                studentId: cell.studentId,
                classGroupId: selectedExam.classGroupId,
                classTestId: selectedExam.classTestId,
                examId: selectedExam.id,
                classLessonId: selectedExam.classLessonId,
                score,
                totalScore,
                source: StudentTestScoreSource.MANUAL,
                memo: cell.value,
              },
            });
          }
          ...
          if (!cell.value) {
            await tx.scoreRecord.deleteMany({ where: { studentId: cell.studentId, date: cell.date, title: "???" } });
          } else {
            const numericScore = Number(cell.value);
            ...
            await tx.scoreRecord.upsert({
              ...
              create: { academyId: user.academyId, studentId: cell.studentId, date: cell.date, title: "???", score, maxScore: 100, memo: cell.value },
            });
          }
        }
      }
  ...
  await recordActivity({ actor: user, action: "BULK_UPDATE", entityType: "Student", summary: "Lesson cells saved: " + allowedCells.length, metadata: { count: allowedCells.length, studentIds, selectedClassTestId, examIds, classTestIds } });
...
export async function saveClassLessonConfig(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    const submittedPositions = lessons.map((lesson) => lesson.position);
    await tx.classLesson.deleteMany({
      where: {
        academyId: user.academyId,
        classGroupId,
        position: { notIn: submittedPositions },
      },
    });

    for (const lesson of lessons) {
      await tx.classLesson.upsert({
        where: { classGroupId_position: { classGroupId, position: lesson.position } },
        update: {
          title: lesson.title,
